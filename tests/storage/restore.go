...
var _ = Describe(SIG("VirtualMachineRestore Tests", func() {
	...
	Context("[storage-req]", decorators.StorageReq, func() {
		Context("[test_id:clone]With a more complicated VM", decorators.RequiresSnapshotStorageClass, func() {
			var (
				newVmName            string
			...
			DescribeTable("should restore a vm that boots from a datavolumetemplate", decorators.StorageCritical, func(restoreToNewVM bool) {
				...
				Entry("[test_id:a2]to a new VM", true),
			)
			...
			DescribeTable("should restore a vm with containerdisk and blank datavolume", func(restoreToNewVM bool) {
				...
				Entry("[test_id:newvm]to a new VM", true),
			)
			...
			It("[test_id:a1]should restore with volume restore policy InPlace and DV template as disk", func() {
				// Create a VM and snapshot it
				vm, vmi = createAndStartVM(renderVMWithRegistryImportDataVolume(cd.ContainerDiskAlpine, snapshotStorageClass))
				By(creatingSnapshot)
				snapshot = createSnapshot(vm)
			...
			It("[test_id:restoreinplace]should restore with volume restore policy InPlace and PVC as disk", func() {
