...
var _ = Describe(SIG("Volumes update with migration", decorators.RequiresTwoSchedulableNodes, decorators.VMLiveUpdateRolloutStrategy, func() {
	...
	Describe("Update volumes with the migration updateVolumesStrategy", func() {
		...
		Context(" destination PVC expansion", decorators.StorageReq, decorators.RequiresVolumeExpansion, func() {
			DescribeTable("should migrate the source volume from a source DV to a destination PVC", func(mode string) {
				...
				Entry("[test_id:migration4][test_id:migrate]to a filesystem volume", fsPVC),
		...
		DescribeTable("should migrate the source volume from a source DV to a destination DV", func(allowPostCopy bool) {
			...
			Entry("[test_id:migration9][test_id:migrate]with pre-copy", false),
		...
		It("[test_id:migration5][test_id:migrate]should trigger the migration once the destination PVC exists", func() {
			volName := "disk0"
			srcDV := createDV()
			vm := createVMWithDV(srcDV, volName)
			By("Update volumes")
		...
		It("[test_id:migration8][test_id:migrate]should migrate the source volume from a source and destination block RWX DVs", decorators.StorageCritical, decorators.RequiresRWXBlock, func() {
			volName := "disk0"
		...
		It("[test_id:migration3][test_id:migrate]should migrate the source volume from a block source and filesystem destination DVs", decorators.RequiresBlockStorage, func() {
			volName := "disk0"
		...
		It("[test_id:migration6][test_id:migrate]should migrate a PVC with a VM using a containerdisk", func() {
			volName := "volume"
		...
		It("[test_id:migration7][test_id:migrate]should cancel the migration and clear the volume migration state", func() {
			volName := "volume"
			dv := createDV()
			vm := createVMWithDV(dv, volName)
			createUnschedulablePVC(destPVC, ns, size)
	...
	Context("Hotplug volumes", func() {
		...
		DescribeTable("should be able to add and remove a volume with the volume migration feature gate enabled", func(persist bool) {
			...
			Entry("[test_id:a3]with a persistent volume", true),
		...
		Context("should be able to volume migrate a VM", func() {
			...
			It("[test_id:migration1][test_id:migrate]with a containerdisk and a hotplugged volume", func() {
				const volName = "vol0"
				ns := testsuite.GetTestNamespace(nil)
			...
			DescribeTable("with a datavolume and an hotplugged datavolume migrating", func(srcBlock, dstBlock bool) {
				...
				vmi := libvmi.New(
					...
					libvmi.WithResourceCPU("1"), libvmi.WithResourceMemory("512Mi"),
					libvmi.WithLimitCPU("1"), libvmi.WithLimitMemory("512Mi"),
